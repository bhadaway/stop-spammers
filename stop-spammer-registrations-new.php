...
// Login Attempts: check login attempts & lock users after threshold
function ss_authenticate( $user, $username, $password ) {
	$field = is_email( $username ) ? 'email' : 'login';
	$time = time();
	$options = ss_get_options();
	if ( ! $userdata = get_user_by( $field, $username ) )
		return $user;
	if ( ss_is_user_locked( $userdata->ID ) && $options['ss_login_attempts'] === 'Y'  ) {
		if ( $expiration = ss_get_user_lock_expiration( $userdata->ID ) ) {
	...
	} elseif ( is_wp_error( $user ) && 'incorrect_password' == $user->get_error_code() && $options['ss_login_attempts'] === 'Y' ) {
		ss_add_failed_login_attempt( $options, $userdata->ID );
		$attempts = get_user_meta( $userdata->ID, 'ss_failed_login_attempts', true);
		if ( count( $attempts ) >= ( $options['ss_login_attempts_threshold'] * 2 ) ) {
			$lockout_expiry = '+' . $options['ss_login_lockout_duration'] . ' ' . $options['ss_login_lockout_unit'];
			$expiration = strtotime( $lockout_expiry );
			ss_lock_user( $userdata->ID, $expiration );
...
add_action( 'authenticate', 'ss_authenticate', 100, 3 );

function ss_add_failed_login_attempt( $options, $user_id ) {
	$new_attempts = array();
	$threshold = '-'. $options['ss_login_attempts_duration'] . ' ' . $options['ss_login_attempts_unit'];
	$threshold_date_time = strtotime( $threshold );

	$attempts = get_user_meta( $user_id, 'ss_failed_login_attempts', true );
	$attempts[] = array( 'time' => time(), 'ip' => $_SERVER['REMOTE_ADDR'] );
	foreach( $attempts as $a ) {
		if( $threshold_date_time < $a['time'] )
			$new_attempts[] = $a;
	}
	update_user_meta( $user_id, 'ss_failed_login_attempts', array() );
	update_user_meta( $user_id, 'ss_failed_login_attempts', $new_attempts );
}

function ss_is_user_locked( $user_id ) {
	if ( get_user_meta( $user_id, 'ss_is_locked', true ) == false)
		return false;
	if ( ! $expires = ss_get_user_lock_expiration( $user_id ) )
		return true;
	$time = time();
	if ( $time > $expires ) {
		ss_unlock_user( $user_id );
		return false;
	}
	return true;
}

function ss_get_user_lock_expiration( $user_id ) {
	return get_user_meta( $user_id, 'ss_lock_expiration', true );
}

function ss_lock_user ( $user_id, $expiration ) {	
	update_user_meta( $user_id, 'ss_is_locked', true );
	update_user_meta( $user_id, 'ss_lock_expiration', $expiration );
	update_user_meta( $user_id, 'ss_failed_login_attempts', array() );
}

function ss_unlock_user( $user_id ) {
	update_user_meta( $user_id, 'ss_is_locked', false );
	update_user_meta( $user_id, 'ss_lock_expiration', '' );
	update_user_meta( $user_id, 'ss_failed_login_attempts', array() );
}
