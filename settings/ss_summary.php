...
This plugin works best with the Cloudflare plugin when yout website is using Cloudflare.
	...
	if ( $spcount > 0 ) {
		?>
        <p>Stop Spammers has stopped <span class="green"><?php echo $spcount; ?></span> spammers
...
// count is in data[0] and use the plugin name
    ...
    <p>This address is invalid for testing for the following reason:
        ...
        If you working on a local installation of WordPress, this might be OK. However, if the plugin reports that your
        IP is invalid it may be because you are using Cloudflare or a proxy server to access this page. This will make
        it impossible for the plugin to check IP addresses. You may want to go to the Stop Spammers Testing page in
        order to test all possible reasons that your IP is not appearing as the IP of the machine that your using to
        browse this site.<br />
        It is possible to use the plugin if this problem appears, but most checking functions will be turned off. The
        plugin will still perform spam checks which do not require an IP.<br />
        If the error says that this is a Cloudflare IP address, you can fix this by installing the Cloudflare plugin. If
        you use Cloudflare to protect and speed up your site then you MUST install the Cloudflare plugin. This plugin
...
?>
    ...
    <h3>Paid Support</h3>
    <p>If you need more advanced help securing your site or other web services, and are interested in hiring me, please <a href="mailto:bhadaway@gmail.com">email me</a>. — <em>Bryan</em></p>
