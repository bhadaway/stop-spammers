...
<table>
<tr><td colspan="2">
  <h3><div class="section-title"><?=__('Flags') ?></div></h3>
...
<tr>  <td>    <?php echo __('... approved comments.') ?>  </td>
  <td align="left" width=250>
    <label for="flag_approve_no">
      <input id="flag_approve_no" type="radio" name="f_approve" value="no" <?php if (isset($_POST['f_approve']) and $_POST['f_approve'] === 'no') {
    echo 'checked';
} ?>  />
      <?=__('No') ?></label>
    <label for="flag_approve_yes">
      <input id="flag_approve_yes" type="radio" name="f_approve" value="yes" <?php if (isset($_POST['f_approve']) and $_POST['f_approve'] === 'yes') {
    echo 'checked';
} ?>  />
      <?=__('Yes') ?>
    </label>    <label for="flag_approve_nomatter">      <input id="flag_approve_nomatter" type="radio" name="f_approve" value="0" <?php echo empty($_POST['f_approve']) ? 'checked' : '' ?> />      <?=__('Ignore') ?></label>
  </td>
</tr>
...
?>
  <td>    <?php echo __('... spam comments.') ?>  </td>
  <td align="left">
    <label for="flag_has_spam_no">
      <input id="flag_has_spam_no" type="radio" name="has_spam" value="no"  <?php if (isset($_POST['has_spam']) and $_POST['has_spam'] === 'no') {
    echo 'checked';
} ?>  />
    <?=__('No') ?></label>
    <label for="flag_has_spam_yes">
      <input id="flag_has_spam_yes" type="radio" name="has_spam" value="yes" <?php if (isset($_POST['has_spam']) and $_POST['has_spam'] === 'yes') {
    echo 'checked';
} ?> />
    <?=__('Yes') ?></label>    <label for="flag_has_spam_nomatter">      <input id="flag_has_spam_nomatter" type="radio" name="has_spam" value="0" <?php echo empty($_POST['has_spam']) ? 'checked' : '' ?> />    <?=__('Ignore') ?></label>
  </td>

</tr>
<tr>  <td>    <?php echo __('... the same first and last name.') ?>  </td>  <td align="left" width=250>    <label for="ss_check_name_no">      <input id="ss_domain_no" type="radio" name="ss_check_name" value="no"   <?php echo empty($_POST['ss_check_name']) ? 'checked' : '' ?>  <?php if (isset($_POST['ss_check_name']) and $_POST['ss_domain'] === 'no') {    echo 'checked';} ?> />      <?=__('No') ?></label>    <label for="ss_check_name_yes">      <input id="ss_check_name_yes" type="radio" name="ss_check_name" value="yes" <?php if (isset($_POST['ss_check_name']) and $_POST['ss_check_name'] === 'yes') {    echo 'checked';} ?> />      <?=__('Yes') ?>    </label>  </td></tr>
<tr>  <td>    <?php echo __('... specified top level domain(s). [Separate TLDs by a comma]') ?>  </td>
  <td align="left" width=250>
    <label for="ss_domain_no">
      <input id="ss_domain_no" type="radio" name="ss_domain" value="no"   <?php echo empty($_POST['ss_domain']) ? 'checked' : '' ?>  <?php if (isset($_POST['ss_domain']) and $_POST['ss_domain'] === 'no') {
    echo 'checked';
} ?> />
      <?=__('No') ?></label>
    <label for="ss_domain_yes">
      <input id="ss_domain_yes" type="radio" name="ss_domain" value="yes" <?php if (isset($_POST['ss_domain']) and $_POST['ss_domain'] === 'yes') {
    echo 'checked';
} ?> />
      <?=__('Yes') ?>
    </label>
  ...
  <td>
    <textarea cols="100" rows="2" name="ss_domain_text" ><?php echo isset($_POST['ss_domain_text']) ? htmlspecialchars($_POST['ss_domain_text']) : '' ?></textarea>
  </td>
</tr>

<tr valign="top">
  <td colspan="2">
    <h3><div class="section-title"><?=__('Filters') ?></div></h3>
    <hr width=50% align="left"/>
    <label for="usernameFilter"><?php echo __('Username') ?></label>    
    ...
    <br />
    <small><?php echo __('Refine list by a username (e.g. test, example, etc.)') ?></small>
  </td>
</tr>
 <tr valign="top">
  <td colspan="2">
...
?>
    ...
    <?php echo __('days ago.') ?></label><br />
    <small><?php echo __('Users need time to begin commenting. This filter can show recent registrations.') ?></small>
  </td>
</tr>

<tr valign="top">
  <td colspan="2">
    <label for="f_lastlogin"><?php echo __('Last time user logged in is more than') ?>
...
?>
    ...
    <small><?php echo __('Search by last login.') ?></small>
  </td>
</tr>
...
<tr>
  <td align="left" colspan="2">
    <h3><div class="section-title"><?=__('Table formatting') ?></div></h3>
    <hr width=50% align="left"/>
    <label for="sort_order"><?php echo __('Show') ?></label>
...
?>
    </select> <?php echo __('records') ?><br />
    <small><?php echo __('Max sent allowed is') . ' ' . ini_get('max_input_vars') . ' ' . __('input vars.') ?></small>
  </td>
</tr>
<tr><td colspan="2">
  <input class="button-primary" type="submit" value="<?php echo __('Search') ?>"  name='ss_search'/>
