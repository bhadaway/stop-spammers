...
?>
    <div id="ss-plugin" class="wrap">
        ...
        <p>Plugins often don't clean up their mess when they are uninstalled. Some malicious themes and plugins use
            WordPress options to store some information.</p>
        <p>This function allows you inspect and delete orphan or suspicious options and to change plugin options so that
            they don&acute;t autoload.</p>
        <p>In WordPress, some options are loaded whenever WordPress loads a page. These are marked as autoload options.
            This is done to speed up WordPress and prevent the programs from hitting the database every time some plugin
            needs to look up an option. Automatic loading of options at startup makes WordPress fast, but it can also
            use up memory for options that will seldom or never be used.</p>
        <p>You can safely switch options so that they don&acute;t load automatically. Probably the worst thing that will
        ...
        <p>The change autoload column shows current autoload state. Yes means it is autoloading. No means that it is
            not.</p>
        <p>You can change the autoload settings or delete an option on the form below. Be aware that you can break some
            plugins by deleting their options. I do not show most of the built-in options used by WordPress. The list
            below should be just plugin options.</p>
        <p>It is far safer to change the autoload option value to &quot;no&quot; than to delete an option. Only delete
            an option if you are sure that it is from an uninstalled plugin. If you find your pages slowing down, turn
            the autoload option back to &quot;yes&quot;.</p>
        <p>Option names are determined by the plugin author. Some are obvious, but some make no sense. You may have to
...
//if ($autoload=='no') $au='No';
                    ...
                    <tr bgcolor="#fff">
                        ...
                        <td>
                            <button type="submit" name="view" value="<?php echo $option_name; ?>">view</button>
                        </td>
                    </tr>
					<?php
				}
