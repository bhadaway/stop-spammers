...
<div id="ss-plugin" class="wrap">
	...
	<?php if ( ! empty( $msg ) ) {
		echo "<span style=\"color:red;size=2em;\">$msg</span>";
	} ?>
    <form method="post" action="">
        ...
        <fieldset>
            ...
            <p>This message is only visible to spammers. It only shows if spammers are rejected at the time the login or
                comment form is displayed. You can use the shortcode <em>[reason]</em> to include the deny reason code
                with the message. You can also use <em>[ip]</em> in your message which would be the user's IP address.
                (You may not want to give spammers hints on how they were denied.)</p>
            <textarea id="rejectmessage" name="rejectmessage" cols="40"
                      rows="5"><?php echo $rejectmessage; ?></textarea>
        </fieldset>
        <br />
        <fieldset>
            <legend><span style="font-weight:bold;font-size:1.2em">Send Spammer to Another Web Page</span></legend>
            Enable redirect:
            <input type="checkbox" name="redir" value="Y" <?php if ( $redir == 'Y' ) {
        ...
        <fieldset>
			...
			?>
            <p>The plugin is extremely aggressive and will probably block some small number of legitimate users. You can
                ...
                <input type="radio" value="G" name="chkcaptcha" <?php if ( $chkcaptcha == 'G' ) {
					echo "checked=\"checked\"";
				} ?>/>
                Google reCAPTCHA<br />
                <input type="radio" value="S" name="chkcaptcha" <?php if ( $chkcaptcha == 'S' ) {
					echo "checked=\"checked\"";
				} ?>/>
        ...
        <fieldset>
            ...
            <p>These API keys are used for displaying a Google reCAPTCHA on your site.
                ...
                You can register and get an API key at <a href="https://www.google.com/recaptcha/admin#list"
