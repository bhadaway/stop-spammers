...
class chkvalidip {
	...
	public function process( $ip, &$stats = array(), &$options = array(), &$post = array() ) {
		if ( empty( $ip ) ) {
			__( 'Invalid IP: ', 'stop-spammer-registrations-plugin' ) . $ip;
		}
		...
		if ( $ip == '127.0.0.1' ) {
			__( 'Accessing Site Through localhost', 'stop-spammer-registrations-plugin' );
		}
		$priv = array(
		...
		*/
		...
		if ( array_key_exists( 'SERVER_ADDR', $_SERVER ) ) {
			...
			if ( $ip == $lip ) {
				__( 'IP Same as Server: ', 'stop-spammer-registrations-plugin' ) . $ip;
			}
		...
		if ( substr( $ip, 0, $j ) == substr( $lip, 0, $k ) ) {
			__( 'IP same /24 subnet as server ', 'stop-spammer-registrations-plugin' ) . $ip;
		}
		return false;
	}
}
